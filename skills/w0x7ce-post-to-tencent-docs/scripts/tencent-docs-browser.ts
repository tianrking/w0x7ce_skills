...
async function getShareLink(session: ChromeSession): Promise<string> {
  ...
  // Try to click the share button to get more options
  const shareButtonFound = await session.cdp.send<{ result: { value: any } }>('Runtime.evaluate', {
    expression: `
      (function() {
        const all = document.querySelectorAll('*');
        for (const el of all) {
          const text = (el.textContent || '').trim();
          // Look for share/invite buttons in the header area
          if ((text.includes('邀请') || text.includes('分享') || text === 'Invite' || text === 'Share') &&
              el.offsetParent !== null && text.length < 20) {
            const rect = el.getBoundingClientRect();
            // Check if it's in the top area of the page (header)
            if (rect.y < 150) {
              return {
                found: true,
                text: text,
  ...
  if (shareButtonFound.result.value?.found) {
    const { text: buttonText, x: btnX, y: btnY } = shareButtonFound.result.value;
    console.log(`[tencent-docs] Found share button: ${buttonText}`);
    ...
    // Look for "复制链接" button
    const copyButtonFound = await session.cdp.send<{ result: { value: any } }>('Runtime.evaluate', {
      expression: `
        (function() {
          const all = document.querySelectorAll('*');
          for (const el of all) {
            const text = (el.textContent || '').trim();
            if ((text === '复制链接' || text === '复制' || text === 'Copy link') &&
    ...
    if (copyButtonFound.result.value?.found) {
      const { x: copyX, y: copyY } = copyButtonFound.result.value;
      console.log(`[tencent-docs] Found copy button: ${copyButtonFound.result.value.text}`);
      ...
      console.log('[tencent-docs] Share link copied!');
    }
  } else {
    console.log('[tencent-docs] Share button not found, using document URL as share link');
  }
